...
import { setRecipes, setLoading, setPosts, setPostsLoading } from "../slice/recipeslice";
...
const Recipes = () => {
  ...
  const { recipes, loading, posts, postsLoading } = useSelector((state) => state.recipes);
  ...
  const sortedData = [...filteredData];
  if (sort === "lowtohigh") sortedData.sort((a, b) => a.rating - b.rating);
  else if (sort === "hightolow") sortedData.sort((a, b) => b.rating - a.rating);
  else if (sort === "atoz") sortedData.sort((a, b) => a.name.localeCompare(b.name));
  else if (sort === "ztoa") sortedData.sort((a, b) => b.name.localeCompare(a.name));
  ...
  const sortedPosts = [...filteredPosts];
  if (postSort === "atoz") sortedPosts.sort((a, b) => a.title.localeCompare(b.title));
  else if (postSort === "ztoa") sortedPosts.sort((a, b) => b.title.localeCompare(a.title));
  else if (postSort === "lowtohigh") sortedPosts.sort((a, b) => a.views - b.views);
  else if (postSort === "hightolow") sortedPosts.sort((a, b) => b.views - a.views);
    ...
    <div className="details">
      ...
      {sortedData.map((recipe) => (
        <div key={recipe.id}>
          ...
          <h3>Ingredients</h3>
          {recipe.ingredients.map((ing, i) => <p key={i}>{ing}</p>)}

          <h3>Instructions</h3>
          {recipe.instructions.map((step, i) => <p key={i}>{step}</p>)}
          ...
          <button
            onClick={async () => {
              ...
              if (next && !posts) {
                ...
                const res = await fetch(`https://dummyjson.com/posts/user/${recipe.id}`);
                const data = await res.json();
